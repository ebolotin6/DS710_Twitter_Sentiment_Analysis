...
INSERT INTRO
INSERT assumptions about filters
INSERT METHOD of collection (and quantity deduced)
INSERT definition of happiness
INSERT NULL HYPOTHESIS + A. HYPOTHESIS
...
# Add column to both datasets to label each group
...
## Next, we combine both groups into a single dataframe for analysis.
...
# remove users that belong to both groups
combined_df <- combined_df[!(combined_df$user_id %in% excluded_users), c("group","sentiment_vader")]

```

## With our data prepared - next we check out group sizes for this sample.
...
The distributions of both groups are also quite similar. However, the fitness group has a higher interquartile range. This means that the fitness group has tweets with a higher sentiment than the media group. Another observation is that the first quartile (for both groups) ends nearly on the median. This tells us that a majority of the sentiments less than zero are very close to zero. Meaning, the spread is skewed towards a sentiment of zero.
...
A few important details are available. As written before, we notice that the fitness group is nearly 70% bigger than the media group. Also: for the fitness group, ~85% of tweets are equal to or above 0 (neutral) sentiment. For the media group, this number is ~83.4%. The conclusion for both groups is that tweets are overwhelmingly neutral or positive in sentiment.
...
ggplot(combined_df, aes(sentiment_vader, fill = group)) + 
  geom_histogram() + ggtitle("Distribution of sentiment")
...
1. Despite the large difference in group size, both groups have a very similar distribution of sentiments. Meaning, people in both groups tend to express themselves similarly in sentiment. 
...
However, can we trust this test given that we have very small sample sizes and the data does not follow a standard normal distribution?
